...
readonly COMPONENT_NAME="Cortex Ingestion Service"
readonly NAMESPACE="cortex-ingestion"
...
main() {
    ...
    if ! wait_for_deployment "${NAMESPACE}" "cortex-ingestion" "${POD_READY_TIMEOUT}"; then
...
create_secrets() {
    ...
    if kubectl get externalsecret cortex-ingestion-external-secrets -n "${NAMESPACE}" &> /dev/null; then
        local status
        status=$(kubectl get externalsecret cortex-ingestion-external-secrets -n "${NAMESPACE}" -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}')
        
        if [[ "$status" == "True" ]]; then
            log_success "ExternalSecret is synced and ready"
            
            # Verify the K8s secret was created
            if kubectl get secret cortex-ingestion-secrets -n "${NAMESPACE}" &> /dev/null; then
                local key_count
                key_count=$(kubectl get secret cortex-ingestion-secrets -n "${NAMESPACE}" -o jsonpath='{.data}' | jq 'keys | length')
                log_success "Secret cortex-ingestion-secrets exists with ${key_count} keys"
                return 0
            else
            ...
            log_info "Check: kubectl describe externalsecret cortex-ingestion-external-secrets -n ${NAMESPACE}"
            return 1
        fi
    else
...
deploy_application() {
    ...
    log_info "Using image: ${ECR_REGISTRY}/cortex-ingestion:${IMAGE_TAG}"
    ...
    sed -e "s|\${ECR_REGISTRY}|${ECR_REGISTRY}|g" \
        ...
        -e "s|\${CORTEX_INGESTION_IAM_ROLE_ARN}|${CORTEX_INGESTION_IAM_ROLE_ARN:-}|g" \
...
verify_installation() {
    log_info "Verifying Cortex Ingestion installation..."
    ...
    log_success "Cortex Ingestion verification complete"
}
...
print_access_info() {
    ...
    echo "========================================"
    echo ""
    echo "Cortex Ingestion Service:"
    echo "  External URL: https://ingestion.usecortex.ai"
    echo "  Internal URL: http://cortex-ingestion.cortex-ingestion.svc.cluster.local:80"
    echo ""
    echo "Health Check:"
    echo "  kubectl exec -it \$(kubectl get pod -n ${NAMESPACE} -l app.kubernetes.io/name=cortex-ingestion -o jsonpath='{.items[0].metadata.name}') -n ${NAMESPACE} -- curl http://localhost:8000/health"
    echo ""
    echo "View Logs:"
    echo "  kubectl logs -f -l app.kubernetes.io/name=cortex-ingestion -n ${NAMESPACE}"
    echo ""
    echo "Scale Deployment:"
    echo "  kubectl scale deployment cortex-ingestion -n ${NAMESPACE} --replicas=5"
    echo ""
    echo "========================================"
}
