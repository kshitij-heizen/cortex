...
APP_DIR="${PROJECT_ROOT}/cortex-application"
...
readonly LOG_FILE="${LOG_DIR}/cortex-app-deploy-$(date +%Y%m%d-%H%M%S).log"
...
Deploy Cortex Application to EKS cluster.
...
build_docker_image() {
    ...
    local image_uri="${ECR_REGISTRY}/cortex-app:${IMAGE_TAG}"
    ...
    if ! docker build \
        ...
        -t "${image_uri}" \
        -t "${ECR_REGISTRY}/cortex-app:latest" \
        -f Dockerfile \
...
push_docker_image() {
    ...
    if ! docker push "${ECR_REGISTRY}/cortex-app:latest" 2>&1 | tee -a "${LOG_FILE}"; then
...
create_iam_role() {
    ...
    if [[ -z "${CORTEX_APP_IAM_ROLE_ARN:-}" ]]; then
        log_warn "CORTEX_APP_IAM_ROLE_ARN not set"
        ...
        log_info "    --namespace=cortex-app \\"
        log_info "    --name=cortex-app \\"
        ...
        log_success "IAM role configured: ${CORTEX_APP_IAM_ROLE_ARN}"
    fi
...
deploy_manifests() {
    ...
    export ECR_REGISTRY
    export IMAGE_TAG
    export CORTEX_APP_IAM_ROLE_ARN="${CORTEX_APP_IAM_ROLE_ARN:-}"
...
wait_for_rollout() {
    ...
    if ! kubectl rollout status deployment/cortex-app -n cortex-app --timeout=300s 2>&1 | tee -a "${LOG_FILE}"; then
        log_error "Rollout did not complete within timeout"
        log_info "Checking pod status..."
        kubectl get pods -n cortex-app 2>&1 | tee -a "${LOG_FILE}"
        kubectl describe pods -n cortex-app -l app.kubernetes.io/name=cortex-app 2>&1 | tail -50 | tee -a "${LOG_FILE}"
        return 1
    fi
...
verify_deployment() {
    ...
    echo ""
    log_info "Deployment:"
    kubectl get deployment -n cortex-app 2>&1 | tee -a "${LOG_FILE}"

    echo ""
    log_info "Pods:"
    kubectl get pods -n cortex-app -o wide 2>&1 | tee -a "${LOG_FILE}"

    echo ""
    log_info "Services:"
    kubectl get svc -n cortex-app 2>&1 | tee -a "${LOG_FILE}"

    echo ""
    log_info "Ingress:"
    kubectl get ingress -n cortex-app 2>&1 | tee -a "${LOG_FILE}"

    echo ""
    log_info "HPA:"
    kubectl get hpa -n cortex-app 2>&1 | tee -a "${LOG_FILE}"

    local pod_name
    pod_name=$(kubectl get pod -n cortex-app -l app.kubernetes.io/name=cortex-app -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")

    if [[ -n "$pod_name" ]]; then
        echo ""
        log_info "Testing health endpoint..."
        if kubectl exec -n cortex-app "$pod_name" -- curl -s http://localhost:8080/ 2>&1 | tee -a "${LOG_FILE}"; then
...
print_summary() {
    ...
    echo ""
    echo "========================================"
    echo -e "${GREEN}Cortex Application Deployment Complete${NC}"
    echo "========================================"
    echo ""
    echo "Image: ${ECR_REGISTRY}/cortex-app:${IMAGE_TAG}"
    echo ""
    echo "Access:"
    echo "  External: https://api.usecortex.ai"
    echo "  Load Balancer: ${nlb_hostname}"
    echo ""
    echo "DNS Configuration:"
    echo "  Point api.usecortex.ai to: ${nlb_hostname}"
    echo ""
    echo "Commands:"
    echo "  View logs:    kubectl logs -f -l app.kubernetes.io/name=cortex-app -n cortex-app"
    echo "  Scale:        kubectl scale deployment cortex-app -n cortex-app --replicas=5"
    echo "  Restart:      kubectl rollout restart deployment/cortex-app -n cortex-app"
    echo ""
...
main() {
    ...
    echo ""
    echo "========================================"
    echo "Cortex Application Deployment"
    echo "========================================"
    echo ""
    ...
    if [[ "$dry_run" == true ]]; then
        ...
        log_info "Would deploy image: ${ECR_REGISTRY}/cortex-app:${IMAGE_TAG}"
        exit 0
    fi
