#!/bin/bash
# =============================================================================
# Find Orphaned Milvus EBS Volumes
# =============================================================================
# Lists all available (unattached) EBS volumes tagged to milvus-* namespaces,
...
echo -e "${BOLD}Finding all available Milvus EBS volumes in ${REGION}...${NC}"
echo ""

# Get all available EBS volumes tagged to milvus-* namespaces
all_vols=$(aws ec2 describe-volumes \
    ...
    --output json 2>/dev/null | jq -r '.[] | select(.NS != null and (.NS | startswith("milvus-"))) | [.NS, .VolumeId, .PVC // "unknown", (.Size|tostring)] | @tsv' | sort)

if [ -z "$all_vols" ]; then
    echo "No available Milvus EBS volumes found."
    exit 0
fi
